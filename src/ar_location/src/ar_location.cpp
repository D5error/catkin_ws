...
int main(int argc, char** argv)
{
    ...
    arLocation *al = new arLocation(vel_pub, count, target_distance, T_Kp, T_Kd, R_Kp, R_Kd);

    ros::Subscriber odom_sub = nh.subscribe("/odom", SUBSCRIBER_BUFFER_SIZE, &arLocation::odomCallback, al);

    ros::Subscriber pose_sub = nh.subscribe("/ar_pose_marker", SUBSCRIBER_BUFFER_SIZE, &arLocation::markersCallback, al);
